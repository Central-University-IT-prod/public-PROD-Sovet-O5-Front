...
function ShowUserNext() {
    ...
    const ID = item.from.user.id;
    console.log(item)
    
    let Links = [];
    let Ls = item.from.user.profile.links || [];
    for (let i = 0; i < Ls.length; i++) {
        Links.push({value: Ls[i], label: Ls[i]});
    }
    let Skills = [];
    const Sk = item.from.user.profile.skills || [];
    for (let i = 0; i < Sk.length; i++) {
        Skills.push({value: Sk[i], label: Sk[i]});
    }
    const Name = item.from.user.first_name;
        ...
        <div className={styles.Screen}>
            ...
            <main className={styles.List}>
                <div className={styles.Check}>
                    <Block text={"Направление"} Value={item.from.user.profile.position} />
                </div>
                <div className={styles.Check}>
                    <h5>Hard Skills:</h5>
                    <CreatableSelect
                ...
                <Block text={"О себе"} Value={item.from.user.profile.description} />
                <Block text={"Опыт"} Value={item.from.user.profile.experience} />
            </main>
