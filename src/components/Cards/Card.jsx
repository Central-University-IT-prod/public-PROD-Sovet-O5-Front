  ...
  const swiped = (direction, nameToDelete, index) => {
    ...
    if (direction === 'left') {
      ...
      try {
        axios({
            method: 'post',
            url: `https://prod-o5.difhel.dev/users/${FF.id}/dislike`,
      ...
      try {
        axios({
            ...
            url: `https://prod-o5.difhel.dev/users/${FF.id}/like`,
            headers: {
                "authorization": `TGMA ${InitData}`
                // "authorization": `TGMA user=%7B%22id%22%3A1368727604%2C%22first_name%22%3A%22Mark%22%2C%22last_name%22%3A%22%22%2C%22username%22%3A%22difhel%22%2C%22language_code%22%3A%22en%22%2C%22is_premium%22%3Atrue%2C%22allows_write_to_pm%22%3Atrue%7D&chat_instance=-2849971755655524577&chat_type=private&start_param=admin&auth_date=1711910453&hash=9a8b781975fcae53a23c5df195f01d8b7a3d534779fd94b626434877cbf1a21c`
            }
      ...
      try {
        axios({
            method: 'post',
            url: `https://prod-o5.difhel.dev/users/${FF.id}/defer`,
        ...
        {!NoUsers ? <div className={styles.card}>
          {dataBase.reverse().map((character, index) => (
            ...
            <TinderCard
              ...
              onSwipe={(dir) => swiped(dir, FF.name, index)}
              onCardLeftScreen={() => outOfFrame(FF.name, index)}
              
            >
              <div onTouchStart={() => setTimeout(OpenProfile, 500) } className={styles.card}>
                <div className={styles.Tags}>
                  <TagBlock image={position} text={FF?.profile?.position || 'без направления'} />
                  {FF?.team?.members.length >= 2 ? <TagBlock image={InTeam} text={'в команде'} /> : ""}
                  {FF?.profile?.skills &&
                    FF?.profile?.skills.map((item, index) => (
                      <TagBlock key={index} image={Hard} text={item} />
                  ))}
                </div>
                <img className={styles.LogoImg} alt="" src={BASEURL + `/avatars/getAvatar?id=${FF?.id}`} />
                <div  className={styles.NameCard}>{FF?.first_name || 'User'}</div>
                <div className={styles.InfoCard}>Опыт: <span className={styles.InfoCardLight}>{FF?.profile?.experience || 'Данные не указаны'}</span></div>
                <div className={styles.InfoCard}>Описание: <span className={styles.InfoCardLight}>{FF?.profile?.description || 'Данные не указаны'}</span></div>
                {FF?.soft_skills_match > 0 ? <div className={styles.InfoCard}>{FF?.soft_skills_match} обших интересов</div> : ""}
                ...
                <ul className={classNames(styles.InfoCard, styles.Top)}>
                  {FF?.profile?.links && FF?.profile?.links.map((item, index) => (
